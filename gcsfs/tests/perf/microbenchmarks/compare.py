...
def format_throughput(mib_s: Optional[float]) -> str:
    """Format throughput in MiB/s into a human-readable string."""
    return f"{mib_s:.2f} MiB/s" if mib_s is not None else "N/A"
...
def extract_benchmark_metric(
    ...
    if runtime not in (None, "N/A"):
        try:
            ...
            return "Throughput", throughput, "MiB/s", True
...
def _load_single_json(json_path: str) -> Dict[str, Dict[str, Any]]:
    with open(json_path, "r", encoding="utf-8") as f:
        data = json.load(f)
    return {b["name"]: b for b in data.get("benchmarks", []) if "name" in b}


def load_benchmarks(target: str) -> Dict[str, Dict[str, Any]]:
    """Load benchmark results from a file, comma-separated files, or directory, averaging multiple runs."""
    if "," in target:
        paths = [
            Path(t.strip()).expanduser().resolve()
        ...
        if p.is_dir():
            # Every run in the directory is averaged. Pass explicit comma-separated
            # paths instead if only a subset should be included.
            paths = sorted(
                p.glob("**/results.json"), key=lambda x: x.stat().st_mtime, reverse=True
            )
        else:
            paths = [p]

    runs = [_load_single_json(str(path)) for path in paths if path.exists()]
    if not runs:
        raise FileNotFoundError(f"No benchmark results found for: {target}")

    if len(runs) == 1:
        return runs[0]

    all_names = sorted(set().union(*[r.keys() for r in runs]))
    averaged: Dict[str, Dict[str, Any]] = {}
    for name in all_names:
        matching = [r[name] for r in runs if name in r]
        template = json.loads(json.dumps(matching[0]))
        means = [
            m.get("stats", {}).get("mean")
            for m in matching
            if m.get("stats", {}).get("mean") is not None
        ]
        if means:
            template.setdefault("stats", {})["mean"] = sum(means) / len(means)
        mean_runs = [
            float(m["extra_info"]["mean_run"])
            for m in matching
            if m.get("extra_info", {}).get("mean_run") not in (None, "N/A")
        ]
        if mean_runs:
            template.setdefault("extra_info", {})["mean_run"] = sum(mean_runs) / len(
                mean_runs
            )
        averaged[name] = template
...
def generate_markdown_report(
    ...
    lines = [
        ...
        f"**Base:** `{base_ref or 'main'}` | **PR:** `{pr_ref or 'PR'}` | **Threshold:** `{threshold:.1f}%`\n",
        alert,
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Exit with code 1 if regression is detected (default: False)",
    )
    parser.add_argument(
        "--base-ref", default="main", help="Base branch name or commit"
    )
    ...
    if args.fail_on_regression and summary["has_regression"]:
        return 1
    return 0
