...
| `--chunk-sizes` | Comma-separated IO sizes in MB that replace the configured `chunk_sizes_mb` (e.g. `1`). Narrows the IO dimension without editing `configs.yaml`. | No |
| `--reuse-files` | Build read-benchmark files once per distinct (bucket, size, count) and share them across cases instead of rebuilding per case. Off by default. | No |
...
Each side accepts a single `results.json`, a comma-separated list of them, or a
directory to search. Multiple runs are averaged per benchmark name, so results from
several groups can be combined into one comparison.

* `--threshold`: Percentage degradation before a benchmark is flagged (default: `10.0`%).
* `--fail-on-regression`: Exit with code `1` when a regression is flagged. Off by default, so regressions are reported without failing the caller.
* `--output-markdown`: Writes a Markdown summary table for PR comments or CI summaries.

For the automated PR performance pipeline, see the Performance Testing section of the
[developer guide](../../../../docs/source/developer.rst).
