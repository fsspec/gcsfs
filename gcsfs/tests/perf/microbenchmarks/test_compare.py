...
class TestCompare(unittest.TestCase):
    ...
    def test_load_benchmarks_merges_multiple_groups(self):
        """A comma-separated path list is how per-group runs are recombined."""
        with tempfile.TemporaryDirectory() as tmp_dir:
            tmp_path = Path(tmp_dir)
            info_file = tmp_path / "info.json"
            read_file = tmp_path / "read.json"

            info_file.write_text(
                json.dumps(
                    {"benchmarks": [{"name": "info_bucket", "stats": {"mean": 1.0}}]}
                )
            )
            read_file.write_text(
                json.dumps(
                    {"benchmarks": [{"name": "read_seq", "stats": {"mean": 2.0}}]}
                )
            )

            merged = compare.load_benchmarks(f"{info_file},{read_file}")

            self.assertEqual(sorted(merged), ["info_bucket", "read_seq"])
            self.assertEqual(merged["info_bucket"]["stats"]["mean"], 1.0)
            self.assertEqual(merged["read_seq"]["stats"]["mean"], 2.0)
