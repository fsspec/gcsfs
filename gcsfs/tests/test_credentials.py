...
def test_raw_token_credentials_init_with_raw_token_fetches_expiry(mock_token_info_api_response):
    """
    Test that initializing GoogleCredentials with a raw string token 
    triggers the API lookup and sets the expiry.
    """
    future_time = int((datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(seconds=600)).timestamp())
    mock_token_info_api_response.json.return_value = {"exp": str(future_time)}

    with patch("gcsfs.credentials.requests.get", return_value=mock_token_info_api_response) as mock_get:
        creds = GoogleCredentials(project="my-project", token=MOCK_TOKEN_STR, access="read_only")
        ...
        assert creds.credentials.expiry == datetime.datetime.utcfromtimestamp(future_time)

def test_raw_token_credentials_init_env_var_disables_fetch(mock_token_info_api_response):
...
def test_raw_token_credentials_init_raises_on_invalid_token(mock_token_info_api_response):
    ...
    with patch("gcsfs.credentials.requests.get", return_value=mock_token_info_api_response):
        with pytest.raises(ValueError, match="Provided token is either not valid"):
             GoogleCredentials(project="my-project", token="bad_token_string", access="read_only")

def test_raw_token_credentials_refresh_throws_error_after_expiry(mock_token_info_api_response):
    """Tests that raw token cred refresh throws error after expiry."""    
    future_time = datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(seconds=600)
    mock_token_info_api_response.json.return_value = {"exp": str(int(future_time.timestamp()))}
    
    with patch("gcsfs.credentials.requests.get", return_value=mock_token_info_api_response) as _:
        creds = GoogleCredentials(project="my-project", token="my_token", access="read_only")
    ...
    creds.credentials.expiry = datetime.datetime.utcnow() - datetime.timedelta(minutes=10)

    # Refresh after expiry
    with pytest.raises(NonRetryableError, match="Got error while refreshing credentials"):
        creds.maybe_refresh()

def test_raw_token_credentials_init_raises_on_short_lived_token(mock_token_info_api_response):
    """
    ...
    with patch("gcsfs.credentials.requests.get", return_value=mock_token_info_api_response):
        with pytest.raises(ValueError, match="less than the safety buffer"):
             GoogleCredentials(project="my-project", token="short_lived_token", access="read_only")