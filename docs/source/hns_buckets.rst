...
.. list-table:: Operation Semantics: Flat Namespace vs. HNS
   ...
   * - **``mkdir``**
     - Creates a 0-byte marker file ending in a slash (e.g., ``dir/``) via the Objects API.
     - Calls the native GCS Folders API, creating a strongly consistent, physical GCS Folder resource.
   * - **``rmdir`` / ``rm``**
     - Paginates through and individually issues delete requests for every object matching the prefix.
     - Deletes the folder resource directly via the Folders API. **Atomic** and more performant; GCS natively cascades the deletion to all contents server-side.
   * - **``rename`` / ``mv``**
     - Issues a ``Copy`` request for each object under the prefix, followed by ``Delete``. Non-atomic, ``O(N)``.
     - Triggers a single native metadata-only rename on the folder. **Atomic** and more performant, ``O(1)``.
   * - **``ls`` / ``dir``**
     - Simulates listing by scanning object prefixes and using a delimiter (``/``) to group results.
     - Natively hierarchical. API returns exact objects and child folders within the parent without prefix-scanning.
   * - **``info``**
...
2. **``mkdir`` behavior:** Previously, in a flat namespace, calling ``mkdir`` on a path might only ensure the underlying bucket exists or create a simple 0-byte marker. With HNS enabled, calling ``mkdir`` will create an actual folder resource. Furthermore, if you pass ``create_parents=True``, it will physically create all intermediate folder resources along the specified path.
