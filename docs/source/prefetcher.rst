...
Prefetcher is enabled by default via the ``adaptive`` cache type when ``cache_type`` is not set explicitly with `DEFAULT_GCSFS_CONCURRENCY=4`. To select an alternative cache or disable prefetching, pass the standard ``cache_type=`` argument (e.g. ``cache_type="readahead"`` or ``cache_type="none"``).
