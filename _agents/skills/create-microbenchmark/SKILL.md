...
2. **Validation: Check for Existing Benchmarks and Dependencies**
   ...
   - **Direct Benchmark Check:** Use `find_by_name` or `list_dir` to check if a directory `gcsfs/tests/perf/microbenchmarks/<method-name>` or file `test_<method-name>.py` already exists. If it does, **abort** the procedure and inform the user that a benchmark for this method already exists.
...
3. **Scaffold the New Microbenchmark**
   Use the `write_to_file` tool to create the directory `gcsfs/tests/perf/microbenchmarks/<method-name>` and the four following required files:
